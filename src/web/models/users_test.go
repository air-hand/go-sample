...
func testUserToManyGroups(t *testing.T) {
	...
	_, err = tx.Exec("insert into `user_groups` (`user_id`, `group_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `user_groups` (`user_id`, `group_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Name`: `varchar`, `Age`: `tinyint`}
	_           = bytes.MinRead
)
